...
class AB_Screener_DB:
    ...
    def insert_query_mcx(self, df, table_name):
        created_by = "python"
        self.connection_int()
        ...
        df["created_by"] = created_by
        df["active_status"] = '1'
        df = df[['symbol', 'commname', 'category', 'unit', 'trd_date', 'exp_date','openprice', 'hprice', 'lprice', 'closeprice', 'prevclose','prevclosedate', 'volume', 'trdval', 'center', 'oi', 'prevoi', 'oidiff','oichange', 'diff', 'change', 'maxdate', 'market_lot']]
        df["exp_date"] = df["exp_date"].dt.strftime('%Y-%m-%d %H:%M:%S')
        df["trd_date"] = df["trd_date"].dt.strftime('%Y-%m-%d %H:%M:%S')
        df["created_on"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        df = df.astype(str)

        value = list(df.itertuples(index=False, name=None))

        query = "INSERT INTO " + table_name + " (symbol,commname,category,unit,trd_date,exp_date,openprice,hprice,lprice,closeprice,prevclose,prevclosedate,volume,trdval,center,oi,prevoi,oidiff,oichange,diff,changer,maxdate,market_lot,created_on) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
        self.__mycursor.executemany(query, value)
